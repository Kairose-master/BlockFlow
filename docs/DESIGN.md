...
예시 9개 전체의 측정표(시간·크기·가스, 결제/타이머/오라클 포함)는 `pnpm bench` 가 `docs/measurements.md` 로 생성한다.
...
각 Phase 끝에 논문화 가능한 측정(가스 표, 검증 시간, 사용자 테스트 성공률)을 남긴다. 가스 표와 검증 시간은 `pnpm bench` → `docs/measurements.md` 로 자동 생성한다(사용자 테스트 성공률은 사람이 채운다).
