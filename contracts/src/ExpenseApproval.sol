...
contract ExpenseApproval {
    ...
    // ───────────── 시퀀스 플로우 비트 (생성기가 부여) ─────────────
    uint256 private constant F1  = 1 << 0;  // Start        -> T1 submit
    uint256 private constant F2  = 1 << 1;  // T1           -> X1 (XOR: amount>1000?)
    uint256 private constant F3  = 1 << 2;  // X1 [yes]     -> T2 approve
    uint256 private constant F4  = 1 << 3;  // X1 [default] -> A1 (AND split)
    uint256 private constant F5  = 1 << 4;  // T2           -> X2 (XOR: approved?)
    uint256 private constant F6  = 1 << 5;  // A1           -> T3 pay
    uint256 private constant F7  = 1 << 6;  // A1           -> T4 uploadReceipt
    uint256 private constant F8  = 1 << 7;  // T3           -> A2 (AND join)
    uint256 private constant F9  = 1 << 8;  // T4           -> A2
    uint256 private constant F10 = 1 << 9;  // A2           -> End
    uint256 private constant F11 = 1 << 10; // X2 [yes]     -> A1
    uint256 private constant F12 = 1 << 11; // X2 [default] -> End (반려)

    // ───────────── 태스크 식별자 (이벤트/UI 용) ─────────────
    uint8 public constant TASK_SUBMIT  = 1;
    uint8 public constant TASK_APPROVE = 2;
    uint8 public constant TASK_PAY     = 3;
    uint8 public constant TASK_RECEIPT = 4;

    // ───────────── 프로세스 변수 (BPMN dataFields 확장에서 생성) ─────────────
    struct Vars {
        uint256 amount;
        bool approved;
        bytes32 receiptHash; // 오프체인 문서는 해시만 저장
    }
    ...
    /// @notice 인스턴스 생성 + 역할 바인딩 (역할 순서: Requester, Manager, Finance)
        ...
        returns (uint256 id)
    {
        ...
        bytes32[3] memory roles = [ROLE_REQUESTER, ROLE_MANAGER, ROLE_FINANCE];
    ...
    /// T1: 경비 신청 [Requester]  in: F1  out: F2  sets: amount
    function submit(uint256 id, uint256 amount)
        external
        whenNotPaused
    ...
    /// T2: 승인 [Manager]  in: F3  out: F5  sets: approved
    ...
    /// T3: 지급 [Finance]  in: F6  out: F8
    function pay(uint256 id)
        external
        whenNotPaused
        onlyRole(id, ROLE_FINANCE)
    {
        _require(id, F6, TASK_PAY);
        _fire(id, F6, F8, TASK_PAY);
    }

    /// T4: 영수증 제출 [Requester]  in: F7  out: F9  sets: receiptHash
    function uploadReceipt(uint256 id, bytes32 receiptHash)
        external
        whenNotPaused
        onlyRole(id, ROLE_REQUESTER)
    {
        _require(id, F7, TASK_RECEIPT);
        vars[id].receiptHash = receiptHash;
        _fire(id, F7, F9, TASK_RECEIPT);
    }
    ...
    function _step(uint256 id, uint256 m) internal returns (uint256) {
        ...
        while (progressed) {
            ...
            // X1: XOR split  in: F2  out: F3 [amount > 1000] | F4 [default]
            if (m & F2 != 0) {
                m &= ~F2;
                m |= (v.amount > 1000) ? F3 : F4;
                progressed = true;
            }
            // X2: XOR split  in: F5  out: F11 [approved] | F12 [default]
            if (m & F5 != 0) {
                m &= ~F5;
                m |= v.approved ? F11 : F12;
                progressed = true;
            }
            // A1: AND split  in: F4 | F11  out: F6 & F7
            if (m & (F4 | F11) != 0) {
                m &= ~(F4 | F11);
                m |= (F6 | F7);
                progressed = true;
            }
            // A2: AND join  in: F8 & F9  out: F10
            if (m & (F8 | F9) == (F8 | F9)) {
                m &= ~(F8 | F9);
                m |= F10;
                progressed = true;
    ...
    function enabledTasks(uint256 id) external view returns (uint256 bits) {
        ...
        if (m & F6 != 0) bits |= 1 << TASK_PAY;
        if (m & F7 != 0) bits |= 1 << TASK_RECEIPT;
    }
}
