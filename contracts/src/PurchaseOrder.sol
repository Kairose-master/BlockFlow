...
/// @title PurchaseOrder — BPMN "구매 승인" 프로세스에서 자동 생성된 컨트랙트 (예시)
...
contract PurchaseOrder {
    ...
    uint256 private constant F2 = 1 << 1; // T1           -> X1 (XOR: price>10000?)
    uint256 private constant F3 = 1 << 2; // X1 [yes]     -> T2 approve
    uint256 private constant F4 = 1 << 3; // X1 [default] -> T3 order
    uint256 private constant F5 = 1 << 4; // T2           -> X2 (XOR: approved?)
    uint256 private constant F6 = 1 << 5; // X2 [yes]     -> T3 order
    uint256 private constant F7 = 1 << 6; // X2 [default] -> End (반려)
    uint256 private constant F8 = 1 << 7; // T3           -> T4 confirmDelivery
    uint256 private constant F9 = 1 << 8; // T4           -> End

    // ───────────── 태스크 식별자 (이벤트/UI 용) ─────────────
    uint8 public constant TASK_REQUEST  = 1;
    uint8 public constant TASK_APPROVE  = 2;
    uint8 public constant TASK_ORDER    = 3;
    uint8 public constant TASK_DELIVERY = 4;

    // ───────────── 프로세스 변수 (BPMN dataFields 확장에서 생성) ─────────────
    struct Vars {
        bytes32 itemHash; // 오프체인 문서는 해시만 저장
        uint256 price;
        bool approved;
        bytes32 deliveryHash; // 오프체인 문서는 해시만 저장
    }
    ...
    /// @notice 인스턴스 생성 + 역할 바인딩 (역할 순서: Requester, Manager, Buyer, Supplier)
        ...
        returns (uint256 id)
    {
        ...
        bytes32[4] memory roles = [ROLE_REQUESTER, ROLE_MANAGER, ROLE_BUYER, ROLE_SUPPLIER];
    ...
    /// T1: 구매 요청 [Requester]  in: F1  out: F2  sets: itemHash, price
    function request(uint256 id, bytes32 itemHash, uint256 price)
        external
        whenNotPaused
        onlyRole(id, ROLE_REQUESTER)
    {
        _require(id, F1, TASK_REQUEST);
        vars[id].itemHash = itemHash;
        vars[id].price = price;
    ...
    /// T3: 발주 [Buyer]  in: F4 | F6  out: F8
    function order(uint256 id)
        external
        whenNotPaused
        onlyRole(id, ROLE_BUYER)
    {
        _require(id, (F4 | F6), TASK_ORDER);
        _fire(id, (F4 | F6), F8, TASK_ORDER);
    }

    /// T4: 납품 확인 [Supplier]  in: F8  out: F9  sets: deliveryHash
    function confirmDelivery(uint256 id, bytes32 deliveryHash)
        external
        whenNotPaused
        onlyRole(id, ROLE_SUPPLIER)
    {
        _require(id, F8, TASK_DELIVERY);
        vars[id].deliveryHash = deliveryHash;
        _fire(id, F8, F9, TASK_DELIVERY);
    }
    ...
    function _step(uint256 id, uint256 m) internal returns (uint256) {
        ...
        while (progressed) {
            ...
            // X1: XOR split  in: F2  out: F3 [price > 10000] | F4 [default]
            if (m & F2 != 0) {
                m &= ~F2;
                m |= (v.price > 10000) ? F3 : F4;
                progressed = true;
            }
    ...
    function enabledTasks(uint256 id) external view returns (uint256 bits) {
        ...
        if (m & (F4 | F6) != 0) bits |= 1 << TASK_ORDER;
        if (m & F8 != 0) bits |= 1 << TASK_DELIVERY;
    }
}
