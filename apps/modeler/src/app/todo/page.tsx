...
export default function TodoPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-xl font-bold mb-1">할 일</h1>
      <p className="text-sm text-gray-500 mb-4">내가 담당자인 건 중에서 지금 내 차례인 일이에요. 값을 넣고 "완료" 를 누르면 끝.</p>
      {notice && <p className="text-sm text-green-700 mb-3" data-testid="task-done">{notice}</p>}
      {cards.length === 0 && <p className="text-sm text-gray-500" data-testid="no-todo">지금은 할 일이 없어요.</p>}
      {cards.map((c) => (
